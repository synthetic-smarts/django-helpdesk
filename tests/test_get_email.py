...
class GetEmailCommonTests(TestCase):
    ...
    def test_will_delete_ignored_email(self):
        ...
        with self.assertRaises(DeleteIgnoredTicketException):
            extract_email_metadata(message.as_string(), self.queue_public, self.logger, team_id=_HD_TEST_TEAM)
    ...
    def test_email_with_inline_and_multipart_as_attachments(self):
        ...
        extract_email_metadata(base_message.as_string(), self.queue_public, self.logger, team_id=_HD_TEST_TEAM)
...
class GetEmailParseTests(TestCase):
    """Email parsing + team propagation via the SES-style direct entry point
    (extract_email_metadata with a resolved team_id). The POP3/IMAP polling
    transports are fail-closed and tested in GetEmailPollingFailClosedTests; the
    parse assertions (previously run through the transport-parameterized command
    path) are preserved here on the team-aware direct path."""

    def setUp(self):
        self.queue_public = Queue.objects.create(
            title="Basic Queue", slug="QQ",
            allow_public_submission=True, allow_email_submission=True,
        )
        self.logger = logging.getLogger("helpdesk")

    def _process(self, raw_email):
        return extract_email_metadata(
            raw_email, self.queue_public, self.logger, team_id=_HD_TEST_TEAM
        )
    ...
    def test_read_html_multipart_email(self):
        ...
        msg["From"] = me
        msg["To"] = you
        msg["Cc"] = cc_one + ", " + cc_two
        text = "Hi!\nHow are you?\nHere is the link you wanted:\nhttps://www.python.org"
        html = (
            "<html><head></head><body><p>Hi!<br>How are you?<br>"
            "Here is the <a href=\"https://www.python.org\">link</a> you wanted.</p>"
            "</body></html>"
        )
        msg.attach(MIMEText(text, "plain"))
        msg.attach(MIMEText(html, "html"))
        self._process(msg.as_string())
        ticket1 = get_object_or_404(Ticket, pk=1)
...
class GetEmailPollingFailClosedTests(TestCase):
    """POP3/IMAP polling fails closed: it reaches create_object_from_email_message
    with no resolved team and MUST NOT create a tenant row (ADR-0083). SES->S3 is
    our only inbound; the polling helpers are dead-but-present."""

    def setUp(self):
        self.temp_logdir = mkdtemp()
        self.queue = Queue.objects.create(
            title="Polling", slug="POLL", allow_email_submission=True,
            email_box_type="local", email_box_local_dir="/var/lib/mail/helpdesk/",
            logging_dir=self.temp_logdir, logging_type="none",
        )

    def tearDown(self):
        rmtree(self.temp_logdir)

    def test_polling_path_creates_no_ticket_without_team(self):
        raw = "To: helpdesk@example.com\nFrom: x@example.com\nSubject: s\n\nbody"
        with (
            mock.patch("os.listdir", return_value=["f1"]),
            mock.patch("helpdesk.email.isfile", return_value=True),
            ...
            try:
                ...
                pass  # fail-closed may surface as a raised ValueError
        self.assertEqual(Ticket.objects.count(), 0)
        self.assertEqual(FollowUp.objects.count(), 0)

    def test_create_object_requires_team_id(self):
        import inspect

        self.assertIn(
            "team_id",
            inspect.signature(helpdesk.email.create_object_from_email_message).parameters,
        )
        with self.assertRaises(TypeError):
            helpdesk.email.create_object_from_email_message(
                None, None, {}, [], logging.getLogger("helpdesk")
            )
...
class GetEmailCCHandling(TestCase):
    ...
    def test_read_email_cc(self):
        ...
        # SES-style direct ingestion with the resolved team (the POP3/IMAP polling
        # path is fail-closed; see GetEmailPollingFailClosedTests). Reply threads to
        # the original ticket via the [CC-1] subject slug.
        extract_email_metadata(
            test_email, self.queue_public, logging.getLogger("helpdesk"),
            team_id=_HD_TEST_TEAM,
        )
